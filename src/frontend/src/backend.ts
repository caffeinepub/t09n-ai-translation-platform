...
export class Backend implements backendInterface {
    ...
    async isInWaitlist(arg0: string): Promise<boolean> {
        if (this.processError) {
            try {
                const result = await this.actor.isInWaitlist(arg0);
                return result;
            } catch (e) {
                this.processError(e);
    ...
    async waitlistCount(): Promise<bigint> {
        if (this.processError) {
            try {
                const result = await this.actor.waitlistCount();
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
